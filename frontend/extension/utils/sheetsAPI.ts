...
export async function clearHighlights(
  ...
  if (!pairs || pairs.length === 0) {
    console.log("[BugSense] No rows to clear highlights.");
    return;
  }
  ...
  pairs.forEach((p: any) => {
    if (Array.isArray(p)) {
      p.forEach((id: number) => uniqueRows.add(id + 1));
    } else if (p.i !== undefined && p.j !== undefined) {
      uniqueRows.add(p.i + 1);
      uniqueRows.add(p.j + 1);
    }
  ...
  // build requests to reset background
  const requests = [...uniqueRows].map((rowIndex) => ({
    repeatCell: {
      range: {
        sheetId: actualSheetId,
      ...
      cell: {
        userEnteredFormat: {
          backgroundColor: null, // 🧹 removes red fill
        },
      },
      fields: "userEnteredFormat.backgroundColor",
    },
  ...
  if (resp.ok) {
    console.log("[BugSense] Cleared duplicate highlights 🧹");
  } else {
    console.warn("[BugSense] Failed to clear highlights:", await resp.text());
  }
}
